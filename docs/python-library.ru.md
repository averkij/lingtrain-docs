...
aligner.fill_db(
    ...
    proxy_from=[],    # Опционально: прокси-текст для исходного языка
    proxy_to=[],      # Опционально: прокси-текст для целевого языка
...
aligner.align_db(
    ...
    use_proxy_from=False,         # Использовать прокси для исходного языка
    use_proxy_to=False,           # Использовать прокси для целевого языка
...
## Использование прокси-текстов {#proxy}

Прокси-тексты — это машинные переводы, используемые как посредники для малоресурсных языковых пар. Прокси должен иметь то же количество строк, что и оригинальный разбитый текст.

```python
# Загрузка прокси-текстов
with open("source_ba_proxy_en.txt") as f:
    proxy_from = f.read().splitlines()

# Создание базы данных с прокси
aligner.fill_db(
    db_path,
    lang_from="ba",
    lang_to="ru",
...
# Выравнивание с использованием прокси
aligner.align_db(
    db_path,
