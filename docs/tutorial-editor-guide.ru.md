...
Введите точный ID строки или подстроку текста в поле **«ID или подстрока»**, затем нажмите Enter или **«Перейти»**. Поиск по тексту учитывает Unicode и не зависит от регистра. Если найдено несколько строк, редактор выбирает ближайшую к текущей позиции кандидатов (при равном расстоянии — строку с меньшим ID). Неудачный поиск сохраняет текущее окно кандидатов.
...
## Проверка и управление ID {#validation-id-management}

Постоянно видимый раздел **«Проверка и модификация»** проверяет текущую двуязычную структуру:

- Список **«Ячейки без номеров строк»** разбит на страницы по 20 пар. Для каждой пары показаны оба текста и проблемная сторона. Действие **«Присвоить новый ID»** вставляет исходную строку в структурную позицию после ближайшей предыдущей привязанной строки и повторно использует существующую пару редактора.
- **«Открыть в редакторе»** учитывает выбранное число строк на странице, прокручивает страницу к точной паре и переводит на неё клавиатурный фокус.
- **«Модификация структуры»** показывает все условия, мешающие безопасной перенумерации. Сначала устраните пустые, некорректные, отсутствующие, повторяющиеся, неиспользуемые или несогласованные привязки.

Когда препятствий нет, **«Перенумеровать»** независимо присваивает каждой языковой стороне ID `1..N` в текущем сквозном порядке выравнивания. Порядок строк внутри сгруппированных ячеек и метаданные предложений сохраняются. Подтверждайте действие только после проверки смыслового порядка: отменить его нельзя.

Локальные признаки предложений в файле выравнивания переназначаются. Исторические события чтения и SRS во внешнем хранилище могут сохранить старые ID.
