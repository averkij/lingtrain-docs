...
## Создание из загруженных документов {#creating}

Нажмите **«+ Создать»** в правом верхнем углу вкладки «Выравнивания». На первом шаге выберите **«Монокнига»** или **«Выравнивание»**.

![Список выравниваний — пустое состояние](img/alignments-list.en.png)

![Диалог создания](img/create-alignment-dialog.en.png)

### Монокнига {#creating-monobook}

Выберите **«Монокнига»**, чтобы создать приватную книгу для немедленного чтения из одного загруженного документа. Укажите название, выберите исходный документ и нажмите **«Создать книгу»**. По умолчанию выбирается самый новый документ; название берётся из метки `title`, а если её нет — из имени файла.

Для создания используется текущий отредактированный **предпросмотр предложений**, а не исходный загруженный файл. Удалённые предложения не возвращаются, изменения текста и структурные метки сохраняются, а оставшиеся маркеры абзацев определяют группировку текста. Пустые интервалы между строфами, удалённые при подготовке документа, уже недоступны: каждая последовательная группа оставшихся строк стихотворения становится одной строфой, и восстановить удалённые интервалы нельзя.

Новая книга появляется в списке выравниваний и сразу доступна владельцу в веб-читалке и Theseus. Она остаётся приватной, пока владелец явно не поделится ею или не опубликует её.

При выборе монокниги в списке выравниваний открывается LTM-редактор с одной языковой колонкой. Эта же книга остаётся доступной в однопанельной веб-читалке.

### Выравнивание {#creating-alignment}

Выберите **«Выравнивание»**. На втором шаге:
...
- **Подстрочник (из) / Подстрочник (в)** — включить выравнивание через подстрочные переводы. Эти флажки становятся активными после загрузки подстрочных переводов (см. [Подстрочник](proxy.ru.md)).
...
- **Номер абзаца** для первой исходной строки каждой ячейки (тот же номер с единицы, что и в редакторе меток)
...
В списке кандидатов можно искать точный ID строки или подстроку текста без учёта регистра. При успешном поиске обычное окно кандидатов центрируется на ближайшей подходящей исходной строке; при неудачном поиске текущее окно не меняется.

### Проверка и модификация {#validation-modification}

Раздел под редактором показывает пары, в которых левая или правая ячейка не имеет ID исходной строки. Для каждой пары видны оба текста и проблемная сторона; новый ID присваивается без создания дубликата строки редактора. Кнопка **«Открыть в редакторе»** переводит фокус точно на выбранную пару.

В подразделе **«Модификация структуры»** перечисляются препятствия: пустые или некорректные привязки, отсутствующие и повторяющиеся ID, исходные строки без ссылок и расхождения между обработкой и индексом. Кнопка **«Перенумеровать»** доступна, только если каждая исходная строка с обеих сторон используется ровно один раз. Каждая сторона независимо получает ID `1..N` в текущем порядке выравнивания. Действие нельзя отменить.

Перенумерация обновляет локальные признаки предложений в файле выравнивания. Исторические события чтения и SRS, хранящиеся вне файла, могут содержать старые ID; надёжно переписать их невозможно.
